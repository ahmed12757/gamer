...
export default function Home() {
  const [tickets] = useState([
    ...
    },
    {
      ...
      short_description:
        "A free-to-play CCG set in the Warhammer 40K universe.",
  ...
  const allGenres = useMemo(() => {
    const set = new Set();
    tickets.forEach((t) => t.genre && set.add(t.genre));
  ...
  const allPlatforms = useMemo(() => {
    ...
    tickets.forEach((t) => {
      ...
      t.platform.split(",").forEach((p) => set.add(p.trim()));
    });
    return Array.from(set).sort();
  }, [tickets]);
  ...
  const filteredTickets = useMemo(() => {
    let arr = tickets.filter((t) => {
      ...
      const matchQuery =
        query.length === 0 ||
        `${t.title ?? ""} ${t.short_description ?? ""} ${t.publisher ?? ""} ${
          t.developer ?? ""
        }`
          .toLowerCase()
          .includes(query);
      ...
      const matchGenre =
        ...
        (t.genre && selectedGenres.includes(t.genre));

      const platforms = (t.platform || "").split(",").map((p) => p.trim());
      const matchPlatform =
        selectedPlatforms.length === 0 ||
        selectedPlatforms.some((p) => platforms.includes(p));

      return matchQuery && matchGenre && matchPlatform;
    });
    ...
    return arr;
  }, [tickets, searchQuery, selectedGenres, selectedPlatforms, sortBy]);
      ...
      <section className="mb-8">
        <div className="bg-gray-800/40 border border-gray-700 rounded-md p-3 mb-4">
          <div className="flex flex-col lg:flex-row gap-3 lg:items-center">
            <input
              ...
              placeholder="Search by title, description, publisher..."
              className="w-full lg:flex-1 px-3 py-2 rounded bg-gray-900 border border-gray-700 focus:outline-none focus:ring-1 focus:ring-indigo-500"
            />
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
              ...
              <select
                ...
                className="w-full sm:w-auto px-3 py-2 rounded bg-gray-900 border border-gray-700 focus:outline-none focus:ring-1 focus:ring-indigo-500"
              >
          ...
          <div className="mt-4">
            <div className="text-sm text-gray-300 mb-1">Genres</div>
            <div className="flex gap-2 overflow-x-auto pb-2 chips-scroll scroll-smooth min-w-0">
              {allGenres.map((g) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1 rounded-full border text-sm whitespace-nowrap flex-shrink-0 ${
                      active
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
          {currentTickets.map((ticket, idx) => (
            <Card
              key={idx}
              title={ticket.title || ticket.name || `Ticket #${idx + 1}`}
              desc={ticket.short_description || ticket.desc || "No description"}
              tags={ticket.tags || []}
              img={ticket.thumbnail || ticket.game_url}
              id={ticket.id}
            />
          ))}
        </div>
        <div className="flex items-center justify-center gap-3 mt-6 flex-wrap">
          <button
            ...
            className="px-3 py-2 rounded bg-gray-700 hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed text-sm"
          >
            Prev
          </button>
